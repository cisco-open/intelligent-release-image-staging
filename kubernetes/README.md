...
Prepare a dedicated local PVC root owned by `10001:10001` with mode `2770`
before mounting it. Pre-create its `state` child (`/data/state` in the pod),
also owned by `10001:10001`, with exact mode `0700` and setgid explicitly
cleared. Setgid belongs on the volume root; `state` must not pass it into
private authority directories, where inherited `2700` fails the required
`0700` check. Other private files and subdirectories retain their own modes.
The server uses
`fsGroup: 10001` and `fsGroupChangePolicy: OnRootMismatch` to avoid recursive
...
affected files' identities and restore only their exact required modes:
`0600` for deployment records, their lock, and IOx transcripts; exact `0700`
with setgid cleared for `/data/state` and private IOx directories. Keep the
public catalog certificate `/data/config/tls/crt.pem` at `0644`; group-writable
modes such as `0660` or `0664` fail startup validation. Keep the dedicated
