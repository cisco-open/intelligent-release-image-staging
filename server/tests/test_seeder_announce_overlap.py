...
list, non-expiring previous records with nonsecret `record_id`, rotate that
refuses to evict a still-valid previous, and targeted revoke by value/record."""
import re
...
def test_rotate_preserves_old_as_valid_nonexpiring_previous():
    now = 1_000_000
    ...
    assert rec["expires_at"] == 0            # non-expiring until explicit revoke
    ...
    # previous stays valid regardless of age
    assert secrets_store.valid(rec, now + 10_000_000, 0)
