...
GUIDE = (INSTALL / "device-packages.md").read_text()
ANCHOR = "#build-and-publish-the-arm64-iox-package"
# The recipe kept its id when the heading text changed: the H2 now reads
# "Build the packages { #build-and-publish-the-arm64-iox-package }". Split on
# a regex so the trailing attr_list attribute on the heading line is consumed
# with it.
RECIPE = re.split(
    r"^## Build the packages\b.*\n",
...
def test_all_layouts_link_the_shared_arm_recipe():
    ...
    for page in ("separate-docker-hosts.md", "kubernetes.md"):
        assert f"device-packages.md{ANCHOR}" in (INSTALL / page).read_text()
...
def test_split_docker_initializes_public_roots_before_starting_fresh_server():
    guide = (INSTALL / "separate-docker-hosts.md").read_text()
    ...
    assert "activate-signing.md#initialise-instruction-custody" in section
...
def test_single_docker_guide_keeps_browser_identity_across_restarts():
    # The fact has one home now: the certificate page the "Docker on one host"
    # section of this guide links to.
    page = (INSTALL / "certificates-and-tokens.md").read_text()
    assert "tls/console-fallback.pem.age" in page
    assert "reused on restart" in page
    assert "default changes on server restart" not in page
