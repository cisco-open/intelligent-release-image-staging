...
| `IRIS_PEER_TLS_MODE` | `disabled` | `required` enables peer transfer TLS on the server and, when supplied to CLI installers, devices. A saved Console peer-TLS setting overrides the server environment default and is inherited by Console onboarding. Coordinate the setting across the swarm; it applies to all torrents per process. See [Peer payload transport](security.md#peer-payload-transport-boundary). |
...
| `GET /api/v1/settings/peer-tls`, `POST /api/v1/settings/peer-tls` | Read mode/origin status; POST `{mode, expected_mode}` (`disabled` or `required`). Requires no active deployments/jobs; conflicting mode or busy fleet returns 409. Persists the mode and restarts the origin automatically. |
...
* **Identity-bound** (`heartbeat`, `telemetry`, `policy`, `instructions`,
  `instruction-keylist`, `peer-tls`, `token-refresh`): the
...
| `POST /v1/devices/<id>/peer-tls` | Current catalog bearer, identity-bound | `{csr}` enrolls or renews a 24-hour peer certificate; returns `{mode, certificate, ca, renew_before_seconds}`. Uses the preflight-provisioned catalog trust for HTTPS; private keys stay on-device. 409 when disabled, 429 rate-limited, 503 issuer unavailable. This endpoint does not change TLS mode. |
...
| `peer_tls_mode` | `disabled` | `required` enforces peer TLS for every torrent in the device aria2 process, with automatic certificate enrollment and renewal. Requires compatible peers and the server mode enabled; no plaintext fallback. |
