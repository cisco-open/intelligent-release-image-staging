...
| `IRIS_PEER_TLS_MODE` | `disabled` | `required` enables peer transfer TLS on the server and, when supplied to installers, devices. Coordinate the setting across the swarm; it applies to all torrents per process. See [Peer payload transport](security.md#peer-payload-transport-boundary). |
