...
| An audit-export save says exports are disabled | Reload **Settings → Audit export**. Re-enter the complete destination and password, then save again; do not leave the password blank. Exports stay disabled until a complete save succeeds. An export already using the previous complete configuration may finish. |
...
| Guest Shell stages files but instructions report `verifier_missing` | Check that the latest bootstrap and architecture-matched bundle are deployed and the bundled verifier passed its runtime probe. File staging and signed-policy application are separate checks; failure leaves tracker-only operation. See [Device agents](device-agents.md). |
