...
Keep operator guides short and task-oriented. Prefer Console/API workflows;
keep host provisioning, offline custody, and recovery procedures separate.
Use family names on landing pages, with model-specific evidence in Validation.
Check capability claims against implementation and the API contract; distinguish
configured intent, completed jobs, and measured device behavior. Performance
claims need a dated workload and test scope, not an unconditional guarantee.

Run `python3 -m pytest server/tests/test_public_docs_contract.py -q` when editing
introductory content. It checks API examples against registered routes and keeps
the static landing-page fallback consistent with its interactive copy. Also
check changed links/anchors and build the docs; neither check proves that every
technical statement is correct.
