...
| A file shows as `not readable by the server`, or a secret fails to decrypt | The server cannot read the path, or its age identity does not match the encrypted state | Check ownership for uid 10001 and the selected age key, project and volumes. Preserve the existing identity and state. See [Install on one Docker host](../install/one-docker-host.md). |
| Fetching a missing `aria2c` client fails | The matching release asset cannot be reached | A checkout includes both tested clients. Restore the matching checkout or check access to its pinned release, then rerun [Download the tools that build device packages](../install/build-tools.md). Do not bypass checksum verification or substitute an unverified build. |
| A client check reports `CHECKSUM MISMATCH` | The file differs from the client pinned by this checkout | Stop and restore the matching tested client. Never edit `tools/aria2c.sha256` to silence a mismatch. |
