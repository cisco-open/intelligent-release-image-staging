...
def _doc(peers, captured_at=1000.0, **over):
    """A sidecar document shaped exactly as peer-receipt-hook.sh writes one:
...
# ---- parse_receipt_snapshot(): the measurement ---------------------------

class TestParseSnapshot:
    def test_exact_shape_and_totals(self):
        b = tr.parse_receipt_snapshot(_doc([
    ...
    def test_retired_field_names_appear_nowhere(self):
        ...
        blob = json.dumps(tr.parse_receipt_snapshot(
    ...
    def test_unusable_documents_yield_nothing(self, over):
        assert tr.parse_receipt_snapshot(_doc([_peer("10.0.0.7", 9, 0)],
                                              **over)) is None

    @pytest.mark.parametrize("raw", ["", "not json", "[]", '{"a":1}',
                                     b'{"schema":1}', '{"schema":1,'])
    def test_garbage_yields_nothing(self, raw):
        assert tr.parse_receipt_snapshot(raw) is None
    ...
    def test_rpc_error_response_yields_no_block(self):
        ...
        assert tr.parse_receipt_snapshot(doc) is None

    def test_reads_bytes_payload(self):
        b = tr.parse_receipt_snapshot(_doc([_peer("10.0.0.7", 5, 0)]).encode())
    ...
    def test_ignores_the_session_entry_and_entry_order(self):
        ...
        doc["rpc"].reverse()
        b = tr.parse_receipt_snapshot(json.dumps(doc))
    ...
    def test_accepts_a_bare_peer_list(self):
        b = tr.parse_receipt_snapshot(_doc([], rpc=[_peer("10.0.0.7", 8, 0)]))
        assert b["bytes_from_all_senders_total"] == 8
...
class TestParseIncompleteCapture:
    ...
    def test_nonsense_counters_are_not_zeroes(self, bad):
        b = tr.parse_receipt_snapshot(_doc([_peer("10.0.0.7", 5, 0), bad]))
    ...
    def test_malformed_entries_clear_complete(self, entry):
        b = tr.parse_receipt_snapshot(_doc([_peer("10.0.0.7", 5, 0), entry]))
        assert [r["ip"] for r in b["rows"]] == ["10.0.0.7"]
        assert b["complete"] is False
    ...
    def test_ipv6_peers_are_kept(self):
        b = tr.parse_receipt_snapshot(_doc([_peer("2001:db8::7", 42, 0)]))
    ...
    def test_unreadable_bytes_past_the_hard_cap_make_the_total_a_floor(self):
        ...
        b = tr.parse_receipt_snapshot(_doc(peers))
...
class TestParseTruncation:
    ...
    def test_named_rows_are_the_largest_contributors(self):
        b = tr.parse_receipt_snapshot(_doc(self._many(40)))
        assert len(b["rows"]) == tr.RECEIPT_ROWS_CAP
    ...
    def test_ties_break_on_ip_for_determinism(self):
        ...
        first = tr.parse_receipt_snapshot(_doc(peers))["rows"]
        peers.reverse()
        assert tr.parse_receipt_snapshot(_doc(peers))["rows"] == first
    ...
    def test_entries_past_the_hard_cap_are_weighed_not_silently_dropped(self):
        ...
        b = tr.parse_receipt_snapshot(_doc(kept + over))
        assert b["rows_total"] == tr.HOOK_PEER_ROWS_HARD_CAP + 5
        assert b["rows_omitted"] == (
            tr.HOOK_PEER_ROWS_HARD_CAP - tr.RECEIPT_ROWS_CAP + 5)
        ...
        # wire, so complete stays true — the same rule RECEIPT_ROWS_CAP follows.
        assert b["complete"] is True
...
# ---- fold_peer_receipts(): the guards ------------------------------------

class TestFold:
    def test_accepts_and_stores(self):
        tele = {"started_ts": 900.0}
        b = tr.parse_receipt_snapshot(_doc([_peer("10.0.0.7", 500, 0)]))
        assert tr.fold_peer_receipts(tele, b, now=1010.0) is True
        assert tele["peer_receipts"]["bytes_from_all_senders_total"] == 500
    ...
    def test_all_zero_snapshot_is_discarded(self):
        ...
        tele = {"started_ts": 900.0}
        b = tr.parse_receipt_snapshot(_doc([_peer("10.0.0.7", 0, 900)]))
        assert tr.fold_peer_receipts(tele, b, now=1010.0) is False
        assert "peer_receipts" not in tele
    ...
    def test_all_zero_snapshot_never_overwrites_a_real_one(self):
        tele = {"started_ts": 900.0}
        real = tr.parse_receipt_snapshot(
            _doc([_peer("10.0.0.7", 500, 0)], captured_at=1000.0))
        tr.fold_peer_receipts(tele, real, now=1010.0)
        spurious = tr.parse_receipt_snapshot(
            _doc([_peer("10.0.0.7", 0, 0)], captured_at=1500.0))
        assert tr.fold_peer_receipts(tele, spurious, now=1600.0) is False
        assert tele["peer_receipts"]["bytes_from_all_senders_total"] == 500
    ...
    def test_snapshot_from_before_this_transfer_is_refused(self):
        ...
        stale = tr.parse_receipt_snapshot(
            _doc([_peer("10.0.0.7", 500, 0)], captured_at=4000.0))
        assert tr.fold_peer_receipts(tele, stale, now=5100.0) is False
        assert "peer_receipts" not in tele

    def test_snapshot_after_this_transfer_started_is_kept(self):
        tele = {"started_ts": 5000.0}
        b = tr.parse_receipt_snapshot(
            _doc([_peer("10.0.0.7", 500, 0)], captured_at=5000.0))
        assert tr.fold_peer_receipts(tele, b, now=5100.0) is True

    def test_age_bounds_a_snapshot_with_no_started_ts(self):
        old = tr.parse_receipt_snapshot(
            _doc([_peer("10.0.0.7", 5, 0)], captured_at=1000.0))
        assert tr.fold_peer_receipts(
            {}, old, now=1000.0 + tr.RECEIPT_MAX_AGE_S + 1) is False
        assert tr.fold_peer_receipts({}, old, now=1000.0 + 30.0) is True
    ...
    def test_a_capture_from_the_future_is_refused(self):
        ...
        assert tr.fold_peer_receipts(
            {"started_ts": 100.0}, b,
            now=9000.0 - tr.RECEIPT_FUTURE_SKEW_S - 1) is False

    def test_newer_replaces_older_but_not_the_reverse(self):
        tele = {"started_ts": 100.0}
        newer = tr.parse_receipt_snapshot(
            _doc([_peer("10.0.0.7", 900, 0)], captured_at=2000.0))
        older = tr.parse_receipt_snapshot(
            _doc([_peer("10.0.0.7", 100, 0)], captured_at=1000.0))
        assert tr.fold_peer_receipts(tele, newer, now=2100.0) is True
        assert tr.fold_peer_receipts(tele, older, now=2100.0) is False
        assert tele["peer_receipts"]["bytes_from_all_senders_total"] == 900

    @pytest.mark.parametrize("block", [None, {}, "x", 5, {"captured_at": 1}])
    def test_junk_is_refused_without_raising(self, block):
        tele = {}
        assert tr.fold_peer_receipts(tele, block, now=1.0) is False
        assert tele == {}
...
class TestReportBlock:
    ...
    def test_capture_instant_is_not_the_report_instant(self):
        ...
        tele = _state()["img1"]["tele"]
        tr.fold_peer_receipts(tele, tr.parse_receipt_snapshot(
            _doc([_peer("10.0.0.7", 500, 0)], captured_at=1099.5)), now=1100.0)
        rep = _report({"img1": {"copied": True, "tele": tele}})
        assert rep["peer_receipts"]["captured_at"] == 1099.5
    ...
    def test_participation_rows_never_gain_bytes(self):
        """peers[] and peer_receipts.rows[] sit APART, joined by ip at read
        ...
        assert rep["peer_receipts"]["rows"][0]["ip"] == "10.0.0.7"
    ...
    def test_report_rows_are_copies_of_state(self):
        ...
        tr.fold_peer_receipts(tele, tr.parse_receipt_snapshot(
            _doc([_peer("10.0.0.7", 500, 0)], captured_at=1050.0)), now=1100.0)
        rep = _report({"img1": {"copied": True, "tele": tele}})
        rep["peer_receipts"]["rows"][0]["session_bytes_from_peer"] = 1
        assert tele["peer_receipts"]["rows"][0][
    ...
    def test_capture_outside_the_window_costs_only_itself(self):
        ...
        assert tr.fold_peer_receipts(tele, tr.parse_receipt_snapshot(
    ...
    def test_worst_case_report_fits_the_server_store_bound(self):
        ...
        tele["peer_receipts"] = {
            "source": tr.RECEIPT_SOURCE, "captured_at": 1050.0,
            "complete": True,
            "rows": [{"ip": "%d.%d.255.255" % (200 + i // 256, i % 256),
                     ...
                     for i in range(tr.RECEIPT_ROWS_CAP)],
            "rows_total": 512, "rows_omitted": 512 - tr.RECEIPT_ROWS_CAP,
        ...
        assert len(rep["peer_receipts"]["rows"]) == tr.RECEIPT_ROWS_CAP
    ...
    def test_corrupt_stored_block_is_simply_absent(self):
        for junk in ({"source": "guessed"}, {"source": "aria2_session_counters"},
                     "x", 5, []):
            assert "peer_receipts" not in _report(_state(peer_receipts=junk))
...
class TestSidecarIO:
    def test_read_and_remove(self, tmp_path):
        ...
        p.write_text("hello")
        assert iris_agent._take_peer_receipt_sidecar(str(p)) == b"hello"
        assert not p.exists()

    def test_missing_is_the_ordinary_case(self, tmp_path):
        assert iris_agent._take_peer_receipt_sidecar(
            str(tmp_path / "nope.json")) is None

    def test_oversized_is_removed_unread(self, tmp_path):
        p = tmp_path / "img.bin.peers.json"
        p.write_bytes(b"x" * (tr.RECEIPT_MAX_BYTES + 1))
        assert iris_agent._take_peer_receipt_sidecar(str(p)) is None
        assert not p.exists()
    ...
    def test_unusable_document_is_still_removed(self, tmp_path):
        ...
        side = tmp_path / ("img.bin" + tr.RECEIPT_SIDECAR_SUFFIX)
        side.write_text("{not json")
        deps = types.SimpleNamespace(emit=lambda *a: None)
        assert iris_agent._ingest_peer_receipts(deps, {}, str(stage), 1.0) \
            is False
    ...
    def test_discard_before_a_new_download(self, tmp_path):
        side = tmp_path / ("img.bin" + tr.RECEIPT_SIDECAR_SUFFIX)
        side.write_text("{}")
        iris_agent._discard_peer_receipts(str(tmp_path / "img.bin"))
        assert not side.exists()
        iris_agent._discard_peer_receipts(str(tmp_path / "img.bin"))   # no-op

    def test_sidecar_path_is_derived_from_the_staged_file(self):
        assert tr.receipt_sidecar_path("/flash/guest-share/iris/img.bin") == \
    ...
    def test_ingest_folds_and_removes(self, tmp_path):
        ...
        tele = {"started_ts": 900.0}
        assert iris_agent._ingest_peer_receipts(
...
class TestTerminalTickIngest:
    ...
    def test_folded_before_done_ts_freezes_the_report(self, tmp_path):
        stage = tmp_path / "img.bin"
        (tmp_path / ("img.bin" + tr.RECEIPT_SIDECAR_SUFFIX)).write_text(
        ...
        assert tele["peer_receipts"]["captured_at"] == 1099.0
        assert tele["peer_receipts"]["rows"][0]["has_complete_file"] is True
        assert not (tmp_path / ("img.bin" + tr.RECEIPT_SIDECAR_SUFFIX)).exists()
    ...
    def test_absent_sidecar_leaves_the_tick_untouched(self, tmp_path):
        ...
        assert "peer_receipts" not in state["img1"]["tele"]
    ...
    def test_seeding_only_completion_also_ingests(self, tmp_path):
        ...
        assert state["img1"]["tele"][
            "peer_receipts"]["bytes_from_all_senders_total"] == 77
...
class TestHookScript:
    ...
    def test_writes_a_snapshot_the_agent_can_parse(self, tmp_path):
        ...
        side = tmp_path / ("img.bin" + tr.RECEIPT_SIDECAR_SUFFIX)
        block = tr.parse_receipt_snapshot(side.read_text())
        assert block["bytes_from_all_senders_total"] == 46137344
        assert block["complete"] is True
    ...
    def test_republished_atomically_over_an_existing_snapshot(self, tmp_path):
        ...
        with _Rpc(reply) as rpc:
            assert _run_hook(stage, rpc.port).returncode == 0
        assert tr.parse_receipt_snapshot(
