...
@test "make-release perl scrub rewrites SCRUB_USER in a .py file" {
  ...
  run grep -q 'testuser_secret' "$REL_OUT/server/agent.py"
  [ "$status" -ne 0 ]
  grep -q 'admin' "$REL_OUT/server/agent.py"
  # The .conf file must also be scrubbed.
  run grep -q 'testpass_secret' "$REL_OUT/server/config.conf"
  [ "$status" -ne 0 ]
...
@test "an invalid later row mints nothing and writes nothing" {
  ...
  cat > "$CSV" <<'CSVEOF'
device_id,device_ip,vlan,svi_ip,svi_mask,guest_ip
sw1,100.92.9.3,666,100.92.9.125,255.255.255.252,100.92.9.126
sw2,100.92.9.4,667,100.92.9.129,255.255.255.252,100.92.9.130,,extra-column
sw3,100.92.9.5,668,100.92.9.133,255.255.255.252,100.92.9.134
CSVEOF
  run env IRIS_HOST_IP=10.0.0.9 bash "$GEN" "$CSV"
  [ "$status" -ne 0 ]
...
device_id,device_ip,vlan,svi_ip,svi_mask,guest_ip
sw1,100.92.9.3,666,100.92.9.125,255.255.255.252,100.92.9.126
sw1,100.92.9.4,667,100.92.9.129,255.255.255.252,100.92.9.130
CSVEOF
  run env IRIS_HOST_IP=10.0.0.9 bash "$GEN" "$CSV"
  [ "$status" -ne 0 ]
...
@test "installers carrying an enrollment token are written 0700, as is install-all.sh" {
  run env IRIS_HOST_IP=10.0.0.9 bash "$GEN" "$CSV"
  [ "$status" -eq 0 ]
  [ "$(stat -c %a "$OUT/install-100.92.9.3.sh")" = "700" ]
...
@test "installers for devices no longer in the CSV are purged; output is a whole set" {
  ...
  [ -f "$OUT/install-100.92.9.3.sh" ]
  grep -q 'install-100.92.9.3.sh' "$OUT/install-all.sh"
